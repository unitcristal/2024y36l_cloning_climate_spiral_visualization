...
git add .
git commit -m "💾 Update and deploy on $(date '+%Y-%m-%d %H:%M')" || echo "⚠️ main 브랜치에 커밋할 변경사항 없음"
...
git add .
git commit -m "🌀 Auto deploy on $(date '+%Y-%m-%d %H:%M')"
...
cd ..
echo "✅ 배포 완료! GitHub Pages에서 확인하세요."
